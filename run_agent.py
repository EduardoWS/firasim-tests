...
if __name__ == "__main__":
    ...
    time_since_last_ball_movement = 0.0
    ball_inactive_timeout = 5.0
    ...
    while True:
        ...
        # Salvar o modelo a cada save_interval iterações
        if total_iterations % save_interval == 0:
            model_save_path = f"model_iteration_{total_iterations}.zip"
            print(f"Saving model at iteration {total_iterations} to {model_save_path}")
            full_model.save(model_save_path)
        
        

        # Obter observação do ambiente (incluindo a bola)
        observation = reader.get_observation(attack_side, team_color, args.id)

        # Obter informações da bola
        ball_x, ball_y, ball_vx, ball_vy = observation[0], observation[1], observation[2], observation[3]
        ball_velocity = np.array([ball_vx, ball_vy])

        # Verificar se a bola está se movendo
        if np.linalg.norm(ball_velocity) > 0.01:  # Consideramos a bola como em movimento se a norma da velocidade for maior que 0.01
            time_since_last_ball_movement = 0.0
        else:
            time_since_last_ball_movement += elapsed_time

        # Verificar se a bola ficou parada por tempo suficiente (5 segundos)
        if time_since_last_ball_movement > ball_inactive_timeout:
            # Reposicionar a bola no centro do campo
            sender.send_ball(x=0.0, y=0.0, vx=0.0, vy=0.0)  # Altere os valores de x, y, vx, vy conforme necessário
